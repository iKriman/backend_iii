...
public class TransaccionesDiariasProcessor implements ItemProcessor<TransaccionDiaria, TransaccionDiariaProcesada> {
    ...
    public TransaccionDiariaProcesada process(TransaccionDiaria item) {
        ...
        if (idStr.isBlank() || tipo.isBlank()) {
            throw new DatoBancarioInvalidoException("ID o Tipo de transacción están vacíos.");
        }
        ...
        try {
            ...
            log.info("Procesando transacción diaria ID: {} en hilo: {}", id, Thread.currentThread().getName());
            
            return new TransaccionDiariaProcesada(id, fecha, monto, tipo);
        } catch (Exception e) {
            throw new DatoBancarioInvalidoException("Error de formato en transacción diaria ID: " + idStr);
        }
    }
